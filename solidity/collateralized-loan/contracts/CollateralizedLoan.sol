...
contract CollateralizedLoan {
  ...
  function validateLoanCollateral(
    ...
    uint256 collateralValue = collateralAmount * getPrice(collateralTokenPriceFeed);
  ...
  function validateBorrowerHasCollateral(address borrower, uint256 collateralAmount, address collateralToken) internal view {
  ...
  function getTokenPriceFeed(address token, TokenInfo[] storage tokenList, uint n) internal view returns (address) {
    for(uint k = 0; k < n; k++) {
  ...
  function isTokenInList(address token, TokenInfo[] memory tokenList, uint n) internal pure returns (bool) {
    for(uint k = 0; k < n; k++) {
      if(tokenList[k].addr == token) {
        return true; 
      }
